...
const DIM = 'text-[#557755]';
const HEADER = 'text-[#00cccc]';
const LABEL = 'text-[#888888]';
const ACCENT = 'text-[#ffcc00]';
...
export function generatePillarLines(saju: SajuResult, compact: boolean = false): PillarLine[] {
  ...
  for (const el of elements) {
    ...
    const barLen = Math.round((score / max) * 12);
    ...
    const tag = el === fe.dominant ? ' ◀강' : el === fe.deficient ? ' ◁약' : '    ';
    elBlock.push({
      text: `${EL_SYMBOL[el]} ${padKr(`${elName.korean}(${elName.hanja})`, 7)} ${bar} ${padKr(`${score}`, 3)} ${padKr(`${pct}%`, 4)}${tag}`,
      color: EL_COLOR[el],
    });
  }
  elBlock.push({ text: '' });
  ...
  elBlock.push({ text: `강  ${domEl.korean}(${domEl.hanja}) ${EL_SYMBOL[fe.dominant]}  ━━  약  ${defEl.korean}(${defEl.hanja}) ${EL_SYMBOL[fe.deficient]}`, color: LABEL });
  ...
  gongmangBlock.push({ text: `공망 지지: ${gm.branches.join(', ')}`, color: LABEL });
  if (gm.affectedPillars.length > 0) {
    gongmangBlock.push({ text: `영향 기둥: ${gm.affectedPillars.join(', ')}`, color: YANG_COLOR });
  } else {
    gongmangBlock.push({ text: '해당 기둥 없음 (공망 해소)', color: 'text-[#44cc44]' });
  }

  // ── 일간 블록 ──
  const ilganBlock: PillarLine[] = [];
  ilganBlock.push({ text: '── 일간(日干) 상세 ──', color: HEADER });
  ...
  ilganBlock.push({ text: `일간 비중: ${dmPct}% → ${strength}`, color: dmPct >= 30 ? YANG_COLOR : dmPct >= 15 ? 'text-[#44cc44]' : YIN_COLOR });

  // ── 현재 운세 미니 블록 ──
  const { currentAge: age, currentCycle } = getCurrentLuckInfo(saju);
  const luckBlock: PillarLine[] = [];
  luckBlock.push({ text: '── 현재 운 ──', color: HEADER });
  if (currentCycle) {
    const clEl = ELEMENT_NAMES[currentCycle.pillar.stem.element];
    luckBlock.push({
      text: `대운 ${currentCycle.pillar.ganjiKorean}(${currentCycle.pillar.ganjiHanja}) · ${clEl.korean}(${clEl.hanja})`,
      color: EL_COLOR[currentCycle.pillar.stem.element],
    });
    luckBlock.push({ text: `${currentCycle.startAge}~${currentCycle.endAge}세 구간`, color: LABEL });
  }
  const yl = saju.yearlyLuck;
  const ylEl = ELEMENT_NAMES[yl.pillar.stem.element];
  luckBlock.push({
    text: `세운 ${yl.year}년 ${yl.pillar.ganjiKorean}(${yl.pillar.ganjiHanja}) · ${ylEl.korean}`,
    color: EL_COLOR[yl.pillar.stem.element],
  });

  // compact(모바일): 1열 세로, 데스크톱: 2열 좌우
  if (compact) {
    lines.push(...stackBlocks(elBlock, sinsalBlock, gongmangBlock, ilganBlock, luckBlock));
  } else {
    lines.push(...mergeBlocks(elBlock, sinsalBlock, 38));
    lines.push({ text: '' });
    lines.push(...mergeBlocks(ilganBlock, gongmangBlock, 38));
    lines.push({ text: '' });
    lines.push(...mergeBlocks(luckBlock, [], 38));
    lines.push({ text: '' });
  }
