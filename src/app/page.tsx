...
export default function Home() {
  const {
    ...
    copyCurrentRoom, exportAll, hasAiContent,
  } = useGame();
...
function ExportDialog({ onConfirm, onCancel }: { onConfirm: () => void; onCancel: () => void }) {
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center" onClick={onCancel}>
      ...
      >
        <h3 className="text-[#D4A020] text-sm mb-3 terminal-glow-strong">전체 내보내기</h3>
        <p className="text-[#8A7848] text-[12px] leading-relaxed mb-5">
          모든 방의 사주 풀이를 텍스트 파일(.txt)로 다운로드합니다.
        </p>
        <div className="flex justify-end gap-2">
          ...
          <button
            onClick={onConfirm}
            className="px-4 py-1.5 rounded border border-[#D4A020] bg-[#1a1400] text-[#D4A020] text-[12px] hover:bg-[#2a2000] transition-colors terminal-glow-strong"
          >
            다운로드
          </button>
        </div>
